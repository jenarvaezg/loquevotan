...
def transform():
    ...
    with open(f"{RAW_DIR}/diputados_raw.json", "r") as f:
        ...
        # Normalize and map by p_codi or id
        raw_deps_map = {}
        for d in raw_deps_list:
            d_id = d.get("id") or f"CAT-15-{d.get('p_codi')}"
            raw_deps_map[d_id] = d
            d["id"] = d_id # Ensure it has id
            d["nombre"] = d.get("nombre") or d.get("name")
    ...
    for v in all_raw_votes:
        ...
        if v["titulo"] not in cache and v["titulo"] not in ["Votació", "Votació ordinària", "Votació unànime"]:
    ...
    if titles_to_categorize and api_key:
        print(f"Categorizing {len(titles_to_categorize)} new titles for Catalunya...")
    ...
    for d in raw_deps_list:
        ...
        if d_id not in deputados_all:
            deputados_all[d_id] = {
                ...
                "grupo": d["grupo"],
                "foto": d.get("foto"),
                "provincia": d.get("provincia", "Barcelona")
            }
        grupos_all.add(d["grupo"])
    ...
    for i, v in enumerate(all_raw_votes):
        cat_info = cache.get(v["titulo"], {
            ...
            "etiquetas": [],
            "resumen_sencillo": "Votació al Parlament de Catalunya",
        ...
        for voto in v_list:
            ...
            if dip_id in dip_id_to_idx:
                votos_by_leg[leg_key].append([
                    vot_idx,
                    dip_id_to_idx[dip_id],
                    grupo_to_idx.get(grp_name, -1),
                    code
                ])
            
        # Handle ordinary votes with totals only
        if not v_list and v.get("totales"):
            t = v["totales"]
            favor = t.get("favor", 0)
            contra = t.get("contra", 0)
            abstencion = t.get("abstencion", 0)
            total_emit = t.get("total", 0)
            no_vota = max(0, 135 - total_emit)
        ...
        vot_meta_list.append({
            ...
            "etiquetas": cat_info.get("etiquetas", []) + ["catalunya"],
        ...
        # Calculate group majorities (if possible)
        gm = {}
        ...
        all_tags = cat_info.get("etiquetas", []) + ["catalunya"]
        for tag in all_tags:
            tag_counts[tag] = tag_counts.get(tag, 0) + 1
            
        vot_detail_by_leg[leg_key][vot_idx] = {
            "resumen": cat_info.get("resumen_sencillo", "Votació al Parlament de Catalunya"),
            "textoOficial": v["titulo"],
            "urlCatalunya": "https://www.parlament.cat",
            "group_majority": gm
        }

    # 7. Calculate deputy stats
    dip_stats = []
    vbd = {} # dipIdx -> list of (votIdx, code, grpIdx, leg)
    for leg in LEGISLATURAS:
        for v_entry in votos_by_leg[leg]:
            votIdx, dipIdx, grpIdx, code = v_entry
            if dipIdx is None: continue
    ...
    for di in range(len(diputados_list)):
        ...
        dip_stats.append({
            ...
            "legislaturas": sorted(list(leg_set)) if leg_set else [LEGISLATURAS[0]]
        })
    ...
    manifest = {
        ...
        "heroExamples": [
            ["catalunya", "Tot Catalunya"],
            ["vivienda", "Accés a l'habitatge"],
            ["educacion", "Educació"],
            ["sanitat", "Sanitat"]
        ],
        "latestVotes": [],
